...
@section('content')
    ...
    <div class="space-y-4">
        <div class="flex gap-4 items-center">
            <h1 class="text-2xl font-medium">Edit option</h1>
        </div>
        ...
        <form action="{{ route('admin-option-update', ['id' => $option_id]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="col-span-12 md:col-span-8">
                        <div class="grid grid-cols-12 gap-x-6 gap-y-2">
                            <div class="col-span-12">
                                ...
                                <div class="mt-1">
                                    <input type="text" name="option_name" id="option-name"
                                        value="{{ old('option_name') ?? $option_name }}" placeholder="Home url"
                            ...
                            <div class="col-span-12">
                                ...
                                <div class="mt-1">
                                    <textarea name="option_value" id="option-value" rows="2" placeholder="www.example.com" class="block w-full rounded-md border-0 py-1.5  shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">{{ old('option_value') ?? $option_value }}</textarea>
                                        @error('option_value')
                                        <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                                    @enderror
                                </div>
